...
let rec tokenize buf : Token.token =
    ...
    in let rec proceed (groupSeed : Token.token list list -> Token.token) lines line =
        ...
        let addIdentifier constructor = addToLineProceed(token(constructor(Sedlexing.Utf8.lexeme(buf)))) in
        let rec atom() =
            match%sedlex buf with
                | identifier -> addIdentifier (fun x -> Token.Atom x)
        ...
        in match%sedlex buf with
            ...
            | identifier -> addIdentifier (fun x -> Token.Word x)
            | '.' -> atom()
            | white_space -> skip ()
            | _ -> failwith "Unexpected character"
